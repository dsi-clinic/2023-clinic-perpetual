...
def generate_single_source_of_truth():
    ...
    df_outdoor = df_outdoor.rename(columns={"longitude": "Longitude",
                                            "latitude": "Latitude"})
    ...
    if 'Weekly_Dropoff_Totes' not in df_indoor.columns:
        ...
        galveston_sub = old_galveston.loc[:, ["Name",
                                              "Weekly_Dropoff_Totes",
                                              "Daily_Pickup_Totes"]]
        df_indoor = pd.merge(df_indoor,
                             galveston_sub,
                             on="Name",
                             how='left')
    ...
    source_location = \
        ast.literal_eval(config["original data source"]["source"])
